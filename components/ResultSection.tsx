...
export const ResultSection: React.FC<ResultSectionProps> = ({ 
  ...
  const renderCurrencyPair = (
      sourceVal: number, 
      targetVal: number, 
      textSource: string, 
      textTarget: string,
      labelSource: string = "Nguồn tiền",
      labelTarget: string = "Quy đổi được",
      title?: string
  ) => {
      const formattedSource = formatCurrency(sourceVal, fromCurrency.locale, fromCurrency.code);
      const formattedTarget = formatCurrency(targetVal, toCurrency.locale, toCurrency.code);
        ...
        <div className="flex flex-col gap-4">
            {title && (
                <div className="flex items-center gap-2 px-1">
                    ...
                    <h4 className="text-xs font-extrabold text-primary-700 uppercase tracking-widest">{title}</h4>
                </div>
            )}
            <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="p-4 sm:p-5 rounded-2xl border border-white/50 bg-white/60 backdrop-blur-sm flex flex-col justify-between h-full group/card">
                    ...
                    <div className="flex items-center gap-2 mb-2 opacity-70">
                        ...
                        <span className="text-xs sm:text-sm font-bold uppercase text-slate-500">{labelSource}</span>
                    </div>
                ...
                <div className={`p-4 sm:p-5 rounded-2xl border border-primary-100/50 bg-gradient-to-br from-primary-50/70 to-white/70 backdrop-blur-sm shadow-sm flex flex-col justify-between h-full relative overflow-hidden group/card`}>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`text-xs sm:text-sm font-bold text-primary-500 uppercase`}>{labelTarget}</span>
                    </div>
                    <div className="flex items-start justify-between gap-3">
  ...
  // Calculate Mode: Render 2 distinct sections
  if (calculationData) {
      return (
          <div className="animate-fade-in-up space-y-8 pt-4">
              {renderCurrencyPair(
                  calculationData.total.source,
                  calculationData.total.target,
                  calculationData.total.textSource,
                  calculationData.total.textTarget,
                  "Tổng phí (VND)",
                  "Tổng phí quy đổi",
                  "TỔNG CHI PHÍ DỊCH VỤ (100%)"
              )}
              
              <div className="h-px bg-slate-200/70 w-full mx-auto max-w-sm"></div>

              {renderCurrencyPair(
                  calculationData.stage.source,
                  calculationData.stage.target,
                  calculationData.stage.textSource,
                  calculationData.stage.textTarget,
                  "Phí từng đợt",
                  "Quy đổi từng đợt",
                  "THANH TOÁN MỖI GIAI ĐOẠN (50%)"
              )}
          </div>
      );
  }

  // Convert Mode: Render standard single pair
  return (
