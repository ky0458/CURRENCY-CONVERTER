...
export const NotesManager: React.FC = () => {
  const { notes, tags, isLoading, addNote, updateNoteStatus, deleteNote, deleteNotes, addTag, deleteTag, toggleTagPin } = useNotes();
                        ...
                        <div className="flex flex-col h-full overflow-hidden animate-fade-in-up bg-slate-50/50">
                            ...
                            <div className="bg-white border-b border-slate-100 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] shrink-0 z-20 pt-3 pb-3">
                                ...
                                <div className="px-4 pb-1 pt-1 overflow-x-auto hide-scrollbar flex items-center gap-3 snap-x snap-mandatory scroll-pl-4 select-none touch-pan-x">
                                    ...
                                    <button
                                        ...
                                        className={`
                                            snap-start shrink-0 h-9 px-6 rounded-full text-xs font-bold transition-all duration-300 flex items-center justify-center
                                            ${activeTab === 'all' 
                                                ? 'bg-slate-800 text-white shadow-lg shadow-slate-200 scale-100 ring-2 ring-slate-800 ring-offset-2' 
                                    ...
                                    {tags.map(tag => {
                                        ...
                                        return (
                                            <div 
                                                key={tag.id} 
                                                className={`
                                                    snap-start shrink-0 h-9 flex items-center rounded-full transition-all duration-300 group overflow-hidden
                                                    ${isActive 
                                            ...
                                            >
                                                <button
                                                    ...
                                                    className={`
                                                        px-4 h-full flex items-center gap-2 text-xs font-bold transition-colors
                                                ...
                                                >
                                                    ...
                                                    {/* Pinned Indicator (Always show if pinned, simpler) */}
                                                    {tag.isPinned && (
                                                       <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`w-3 h-3 ${isActive ? 'text-white/80' : 'text-slate-400'}`}>
                                                            <path fillRule="evenodd" d="M11.47 2.47a.75.75 0 0 1 1.06 0l4.5 4.5a.75.75 0 0 1-1.06 1.06l-4.5-4.5a.75.75 0 0 1 0-1.06Zm-5 5a.75.75 0 0 1 1.06 0l4.5 4.5a.75.75 0 0 1 1.06 1.06l-4.5-4.5a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" />
                                                            <path d="M14.75 12.25a.75.75 0 0 0 0 1.5H16v2.25h-2.25a.75.75 0 0 0 0 1.5H16v2.5a.75.75 0 0 0 1.5 0v-2.5h2.25a.75.75 0 0 0 0-1.5H17.5V13.75h2.25a.75.75 0 0 0 0-1.5H14.75Z" opacity="0" /> 
                                                            <path fillRule="evenodd" d="M9.702 3.86a.75.75 0 0 1 .493 1.018l-1.082 3.093 5.925 5.925 3.093-1.082a.75.75 0 0 1 .937.937l-1.383 4.84a.75.75 0 0 1-1.05.474l-3.37-1.444-2.298 2.298a.75.75 0 0 1-1.06 0l-1.061-1.061a.75.75 0 0 1 0-1.061l2.298-2.298-1.444-3.37a.75.75 0 0 1 .474-1.05l4.84-1.383a.75.75 0 0 1 .494-1.018Z" clipRule="evenodd" />
                                                        </svg>
                                                ...
                                                {/* Pin Action */}
                                                {isActive && (
                                                    <div className="flex items-center h-full" style={{ backgroundColor: tag.color }}>
                                                        <div className="h-4 w-px bg-white/30"></div>
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); toggleTagPin(tag.id); }}
                                                            className="h-full px-2.5 flex items-center justify-center hover:bg-black/10 transition-colors text-white"
                                                            title={tag.isPinned ? "Bỏ ghim" : "Ghim"}
                                                        >
                                                            {tag.isPinned ? (
                                                                ...
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-3.5 h-3.5 opacity-80">
                                ...
                                <div className="px-4 py-3 overflow-x-auto hide-scrollbar flex items-center gap-2 snap-x snap-mandatory scroll-pl-4 select-none touch-pan-x">
                                    <button
                                        ...
                                        className={`
                                            snap-start shrink-0 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all border
                                            ${activeStatusFilter === 'all' 
                                                ? 'bg-slate-800 border-slate-800 text-white shadow-md scale-100 ring-2 ring-slate-800 ring-offset-1' 
                                    ...
                                    {(Object.entries(statusConfig) as [NoteStatus, typeof statusConfig[NoteStatus]][]).map(([status, config]) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`snap-start shrink-0 px-3 py-2 rounded-full text-[11px] font-bold transition-all border shrink-0 flex items-center gap-1.5
                                                    ${isActive 
                                                        ? 'shadow-md ring-2 ring-offset-1 scale-100' 
                                                ...
                                                style={{ 
                                                    ...
                                                    boxShadow: isActive ? `0 4px 12px -2px ${config.color}60` : 'none',
                            ...
                            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-50/30 pb-20">
                                {isLoading || isFiltering ? (
                                    ...
                                    <div 
                                        key={`${activeTab}-${activeStatusFilter}`} // Forces re-render animation on filter change
